...
interface Expense {
  ...
  items?: Array<{ name: string; amount: number; sharedBy: string[] }>;
}
...
interface EditExpenseDialogProps {
  ...
  members: Member[];
  expense: Expense;
  onSubmit: (expenseId: string, expenseData: any) => Promise<void>;
}
...
export default function EditExpenseDialog({
  ...
  useEffect(() => {
    if (open && expense) {
      ...
      setSplitMethod(expense.splitMethod as any);

      if (expense.splitMethod === 'percentage') {
        setCustomSplits(expense.splits.map(s => ({ userId: s.userId, value: s.percentage || 0 })));
      } else if (expense.splitMethod === 'exact') {
        setCustomSplits(expense.splits.map(s => ({ userId: s.userId, value: s.amount })));
      } else if (expense.splitMethod === 'itemized' && expense.items) {
